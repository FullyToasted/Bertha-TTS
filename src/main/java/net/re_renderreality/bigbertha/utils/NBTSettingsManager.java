...
public class NBTSettingsManager extends SettingsManager {
	...
	public synchronized void loadSettings() {
		...
		try {
			...
			Reference.logger.info("Error reading command config " + 
	...
	public synchronized void saveSettings() {
		...
		try {
			...
			else Reference.logger.info("Failed to write settings to file " + this.file.getName() + ": settings is not a NBTTagCompound");
		}
		catch (IOException ex) {
			UUID uuid = null; try {uuid = UUID.fromString(this.file.getName().split("\\.")[0]);} catch (IllegalArgumentException e) {}
			EntityPlayerMP player = uuid == null ? null : getPlayerByUUID(uuid);
			
			Reference.logger.info("Error writing command config " + 
