...
public final class Updater implements Runnable {
	 ...
	 */
	public Updater(String mcversion, UpdateCallback listener) {
	...
	public void run() {
		...
		if (versions == null) {Reference.logger.info("No MoreCommands updates for this minecraft version found"); return;}
		...
		if (upd != null) {
			...
			else Reference.logger.info("No MoreCommands updates for this minecraft version found");
		}
		else Reference.logger.info("No MoreCommands updates for this minecraft version found");
	}
	...
	public Map<String, Map<String, Triple<Long, String, String>>> parseManifest(File manifest) {
		...
		try {
			...
			if (nl == null || nl.getLength() <= 0) {Reference.logger.info("Invalid update file"); return null;}
			...
			if (index < 0) {Reference.logger.info("Unaccepted update file version"); return null;}
